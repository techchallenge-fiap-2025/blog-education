...
export class UserController {
  ...
  async getMyProfile(@Req() req) {
    ...
    const { name, role, email } = await this.userService.findOne(userId);
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Lista todos os usuários (apenas para professores)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 403,
    description: 'Acesso negado. Recurso apenas para professores.',
  })
  findAll() {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Busca um usuário específico pelo ID (apenas para professores)',
  })
